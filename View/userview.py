...
def editWO(code):
    if g.user is not None and request.method == 'GET':
        ...
        """同步工单里程"""
        wo = mapi.get_wo_code(code)
        wo.length = wo.car.length
        wo.save()

        return render_template('editWO.html', wo=wo,
...
def okwo():
    if g.user is not None:
        ...
        if key is not None:
            wo_list = mapi.get_ok_wo_list_for_key(key)

        elif startdate is not None and enddate is not None:
            wo_list = mapi.get_ok_wo_list_for_date(start=startdate, end=enddate)

        else:
            wo_list = mapi.get_ok_wo_list()

        return render_template('Settled.html', wo_list=wo_list)
    else:
        return redirect(url_for('login'))


# 零部件出入库记录
@bp.route('/Componenthistory/<code>')
def Componenthistory(code):
    if g.user is not None:
        position = request.args.get('position')
        if position is not None:  # 更新零部件位置信息
            mapi.update_component_position(code, position)
        c = mapi.get_code_component(code)

        c_to_list = mapi.get_c_to_his_list(code)
        c_in_list = mapi.get_c_in_his_list(code)
        return render_template('Componenthistory.html', c=c, c_to_list=c_to_list, c_in_list=c_in_list)
    else:
        return redirect(url_for('login'))
